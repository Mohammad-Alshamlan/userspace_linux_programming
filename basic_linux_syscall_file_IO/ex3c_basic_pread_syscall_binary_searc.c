/*
 * we want to do a binary search in a file with using pread() instead of doing it with lseek() 
 */
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
char *get_word(int fd, char *word, off_t len, off_t *read_offset)
{
	...
	pread (fd, word, max_read, *read_offset);
...
{
	// set the file position to point to the last word if possible -- this is a rough estimate which later will be fixed accordingly!!!
	off_t read_offset=len - MAX_WORD_LENGTH;
...
// [1] we need to know the length of the file
// [2] we need to open the file
// [3] we need to close the file when we are done!!
